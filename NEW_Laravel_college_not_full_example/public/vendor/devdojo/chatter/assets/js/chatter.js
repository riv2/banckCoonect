...
function sendBan()
{
    if($('#ban-modal-period').val() == '' || $('#ban-modal-reason').val() == '')
    {
        alert('Необходимо указать причину длительность бана.');
        return;
    }
    ...
    $.ajax({
        ...
        type: 'POST',
        data: {
            period: $('#ban-modal-interval').val(),
            id: banUserId,
            comment: $('#ban-modal-reason').val()
        },
        success: function(response){

            if(response.status == true)
            {
                $('.ban-icon-' + banUserId + ' i').removeClass('fa-lock-open');
                $('.ban-icon-' + banUserId + ' i').addClass('fa-lock');
                $('.ban-icon-' + banUserId).removeAttr('onclick')
                $('.ban-icon-' + banUserId).removeAttr('style');
            }
            $('#banModal').hide();
        }
    });
