...
@section('content')
	...
	<div id="chatter" class="discussion">
		...
		<div id="chatter_header" class="d-flex padding-b10" style="background-color:{{ $discussion->color }}">
			<div class="container">
				<a class="back_btn" href="{{route('chatter.home')}}"><i class="chatter-back"></i></a>
				<h1>{{ $discussion->title }}</h1>
				<span class="chatter_head_details">
					<span class="hidden_content_mobile">@lang('chatter::messages.discussion.head_details')</span>
					<a class="chatter_cat" href="{{route('chatter.category.show', ['slug' => $discussion->category->slug ])}}" style="background-color:{{ $discussion->category->color }}">{{ $discussion->category->name }}</a>
				</span>
			</div>
		</div>
						...
						@else
								...
								<div class="chatter_sidebar">
									...
									<ul class="nav nav-pills nav-stacked">
										...
										@foreach($categories as $category)
											<li><a href="{{route('chatter.category.show', ['slug' => $discussion->category->slug ])}}"><div class="chatter-box" style="background-color:{{ $category->color }}"></div> {{ $category->name }}</a></li>
										@endforeach
									</ul>
								</div>
								<!-- END SIDEBAR -->
							...
							<div class="col-md-9 right-column">
		                		...
		                		<span class="chatter_posts">
			                		<div class="chatter_avatar">
					        			@if(Config::get('chatter.user.avatar_image_database_field'))
											...
											@if($post->user->hasClientRole() || $post->user->hasAdminRole())
												...
												@endif
												@if($post->user->hasAdminRole() || $discussion->user->hasRight('forum', 'edit'))
												...
												@if(!Auth::guest() && (Auth::user()->hasAdminRole() || Auth::user()->hasTeacherRole() ||Auth::user()->hasRight('forum', 'read')))
													@if(\DevDojo\Chatter\Models\Ban::checkBaned($post->user->id))
														<span class="role-icon pull-right"><i class="fas fa-lock"></i></span>
													@else
														<span class="role-icon pull-right ban-icon-{{ $post->user->id }}" style="cursor: pointer;" onclick="forumBan({{ $post->user->id }})"><i class="fas fa-lock-open"></i></span>
													@endif
												@endif
											</div>
					        		</div>
									...
									@if(!Auth::guest() && (Auth::user()->id == $post->user->id || Auth::user()->hasAdminRole() || Auth::user()->hasTeacherRole() || Auth::user()->hasRight('forum', 'read')))
										...
										<div class="chatter_post_actions desktop">
								...
								@if(!Auth::guest())
									...
									<div id="new_response">
										...
										@if(\DevDojo\Chatter\Models\Ban::checkBaned(Auth::user()->id))
											<div class="alert alert-warning" role="alert">
												Вы забанены по причине: {{ \DevDojo\Chatter\Models\Ban::checkBaned(Auth::user()->id)[0]->comment }} на
												<span class="alert-link">{{\DevDojo\Chatter\Models\Ban::checkBaned(Auth::user()->id)[0]->period }}ч</span>.
											</div>
										@else
													...
													<div id="editor">
														@if( $chatter_editor == 'tinymce' || empty($chatter_editor) )
															<label id="tinymce_placeholder">
																<span>@lang('chatter::messages.editor.tinymce_placeholder')</span>
															</label>
								...
								@else
									...
									<div id="login_or_register">
										<p>
											@lang('chatter::messages.auth', ['home' => route('chatter.home')])
										</p>
									</div>
			...
			<script>
				$('document').ready(function(){
					...
					$('.discussions li').on('click', '.update_chatter_edit', function(e){
						...
						$.form('{{ route('chatter.posts.edit' , ['post' => '']) }}/' + post_id, { _token: '{{ csrf_token() }}', _method: 'PATCH', 'body' : update_body }, 'POST').submit();
					});
					...
					$('.delete_response').click(function(){
						...
						$.form('{{ route('chatter.posts.edit', ['post' => '']) }}/' + post_id, { _token: '{{ csrf_token() }}', _method: 'DELETE'}, 'POST').submit();
					});
