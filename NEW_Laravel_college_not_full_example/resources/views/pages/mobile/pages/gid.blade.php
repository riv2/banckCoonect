            ...
            <div class="card shadow-sm p-3 mb-5 bg-white rounded">
                <div class="card-body">
                    ...
                    @if( (\auth()->user()) && \auth()->user()->hasRole('guest') )
                        ...
                        <p class="lead"> {{ __('You are using the virtual guide MIRAS.APP.') }} </p>
                        <p class="lead"> {{ __('To access the territory, use the QR code.') }} </p>
                        <p class="lead"> {{ __('Purchase goods and services in all Miras.education buildings.') }} </p>
                        <p class="lead"> {{ __('At your service packet Wi-Fi, services of a gym, pool and cafe.') }} </p>
                        <p class="lead"> {{ __('You can pay for services by card of any bank or in cash at the cash desk.') }} </p>
                        <p class="lead text-bold"> {{ __('Answers to the most asked questions HERE:') }} </p>
                        <ul>
                            <li> <h4> {{ __('Should I use the MIRAS.APP app for all purchases?') }} <br>
                                    <small class="text-muted"> {{ __('Yes. Goods and services can only be purchased through MIRAS.APP') }} </small> </h4>
                            </li>
                            <li> <h4> {{ __('If I have not used the entire Internet, will it remain in my account the next day?') }} <br>
                                    <small class="text-muted"> {{ __('Yes, if you do not change the phone number associated with the application.') }} </small> </h4>
                            </li>
                            <li> <h4> {{ __('How is my data stored in MIRAS.APP?') }} <br>
                                    <small class="text-muted"> {{ __('Your account is tied to a phone number.') }} </small> </h4>
                            </li>
                            <li> <h4> {{ __('I am not a Miras.education student, can I purchase goods and services?') }} <br>
                                    <small class="text-muted"> {{ __('Yes. We are glad to all residents and guests of Shymkent.') }} </small> </h4>
                            </li>
                        </ul>
                        <br>
                        <p class="lead"> {{ __('If you have questions, please contact us.') }} </p>
                    ...
                    @if( (\auth()->user()) && \auth()->user()->hasRole('client') )
                        ...
                        @if(app()->getLocale() == 'ru')
                            <video class="col-12 margin-t20 margin-b20" loop autoplay playsinline controls muted="muted" preload="true">
